...
COPY . .
WORKDIR "/src/MicrosoftStoreBadge"
RUN dotnet build "MicrosoftStoreBadge.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "MicrosoftStoreBadge.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "MicrosoftStoreBadge.dll"]